...
for (row1 in 1:nrow(scaled_companies_quote)){
  for (row2 in 1:nrow(scaled_companies_quote)){
    if (scaled_companies_quote[row1, "Symbol"] != scaled_companies_quote[row2, "Symbol"]) {
      val = dist(scaled_companies_quote[c(row1, row2),3:ncol(scaled_companies_quote)])
      finSimilarity_df <- finSimilarity_df |> rbind(data.frame(Ticker1 = scaled_companies_quote[row1, "Symbol"], Ticker2 = scaled_companies_quote[row2, "Symbol"], FinSimilarity = val))
    }
  }
}
